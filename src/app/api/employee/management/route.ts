...
export async function GET(req: NextRequest) {
  try {
    ...
    const rows = await query<RowDataPacket[]>("SELECT id, emp_code, full_name, email_id, mobile_no, designation, status, profile_photo, created_on FROM employees WHERE status=? ORDER BY created_on DESC", ['active']);
