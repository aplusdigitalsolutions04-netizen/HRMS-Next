...
export async function GET(req: NextRequest) {
  try {
    ...
    const counts = await query<RowDataPacket[]>('SELECT status, COUNT(*) as cnt FROM employees GROUP BY status');
    ...
    const newThis = await query<RowDataPacket[]>("SELECT COUNT(*) as cnt FROM employees WHERE created_on >= DATE_SUB(CURDATE(), INTERVAL 30 DAY)");
    const newThisMonth = newThis[0].cnt;
    const prev = await query<RowDataPacket[]>("SELECT COUNT(*) as cnt FROM employees WHERE created_on >= DATE_SUB(CURDATE(), INTERVAL 60 DAY) AND created_on < DATE_SUB(CURDATE(), INTERVAL 30 DAY)");
