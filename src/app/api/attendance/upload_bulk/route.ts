...
export async function POST(req: NextRequest) {
  try {
    ...
    // 1. Scan the first 10 rows for metadata (e.g. "Empcode", "0003", "Name", "Yashi Mishra")
    for (let r = 0; r < Math.min(rows.length, 10); r++) {
      const row = rows[r];
      if (!row) continue;
      for (let c = 0; c < row.length; c++) {
        const cellVal = String(row[c] || '').trim().toLowerCase().replace(/[^a-z0-9]/g, '');
        if (!cellVal) continue;
        
        if (['empcode', 'employeecode', 'empid', 'employeeid', 'code', 'id', 'acno', 'cardno'].includes(cellVal)) {
          if (c + 1 < row.length && row[c + 1]) {
            globalEmpCode = String(row[c + 1]).trim();
          }
        }
        if (['name', 'fullname', 'empname', 'employeename'].includes(cellVal)) {
          if (c + 1 < row.length && row[c + 1]) {
            globalName = String(row[c + 1]).trim();
          }
        }
      }
      if (globalEmpCode && globalName) break;
    }
    ...
    // 2. Search for the daily records header row (the one containing the Date column)
        ...
        if (!matchedKey) {
          for (const [key, patterns] of Object.entries(matchRules)) {
            if (patterns.includes(cellVal)) {
              // Skip matching emp_code or name in the metadata header row
              if (key === 'emp_code' && globalEmpCode && r < 3) continue;
              if (key === 'name' && globalName && r < 3) continue;
              matchedKey = key;
              break;
            }
          }
    ...
    // If we couldn't find a header row containing Date, fallback to first row
    if (headerRowIndex === -1) {
      headerRowIndex = 0;
    ...
    // Verify if we have at least employee code and date mapped
    const mappedKeys = Object.values(colMap);
    const hasEmpCode = mappedKeys.includes('emp_code') || !!globalEmpCode;
    const hasDate = mappedKeys.includes('attendance_date');

    if (!hasEmpCode || !hasDate) {
      const displayHeaders = detectedHeadersList.filter(Boolean).join(', ');
      return jsonError(`Could not identify required columns (Employee Code and Date) in the Excel sheet. Found headers in detected table: [${displayHeaders || 'None'}]. Global Emp Code found: ${globalEmpCode || 'None'}. Please verify that your Excel file has a column named 'Date' and either a column named 'EmpCode' or employee info at the top.`, 400);
    }
    ...
    for (let r = headerRowIndex + 1; r < rows.length; r++) {
      ...
      for (let c = 0; c < row.length; c++) {
        ...
        switch (key) {
          case 'emp_code': emp_code = String(val || '').trim(); break;
      ...
      const existing = await query<RowDataPacket[]>('SELECT id FROM attendance WHERE emp_code = ? AND attendance_date = ?', [emp_code, attendance_date_str]);
      if (existing.length > 0) {
        await execute(
          ...
          [name || null, department || null, day, shift || null, in_time || null, out_time || null, work_plus_ot || null, ot || null, less_hrs || null, working_hours, cleanStatus, remark || null, user.email, 1, existing[0].id]
        );
      } else {
        const newId = uuidv4();
        await execute(
          ...
          [newId, emp_code, name || null, department || null, attendance_date_str, day, shift || null, in_time || null, out_time || null, work_plus_ot || null, ot || null, less_hrs || null, working_hours, cleanStatus, remark || null, user.email, 1]
        );
      }

      affectedSummaries.add(`${emp_code}:${dateObj.getMonth() + 1}:${dateObj.getFullYear()}`);
      importCount++;
    }

    // Recalculate monthly summaries for all affected employees
    for (const item of affectedSummaries) {
      ...
      if (existingSummary.length > 0) {
        await execute(
          ...
          [empName, empDept, fromDate, toDate, presentDays, absentDays, totalHours, existingSummary[0].id]
        );
      } else {
        ...
        await execute(
          ...
          [newSummaryId, empCode, empName, empDept, fromDate, toDate, presentDays, absentDays, totalHours, 'Good', 0]
        );
      }
    }

    return jsonSuccess({ message: `Successfully imported ${importCount} attendance records and updated monthly summaries.` });
  } catch (e: any) {
    console.error('[attendance_bulk_upload] Error:', e);
